...
function fuelPct(current, capacity) {
  ...
  return Math.min(100, Math.round(((current ?? 0) / capacity) * 100))
}
                ...
                <div key={f.id} className={styles.fuelCard}>
                  ...
                  <div className={styles.fuelStats}>
                    <span className={styles.fuelLevel}>
                      {f.currentLevel ?? 0}
                      <span className={styles.fuelCapacity}> / {f.tankCapacity ?? 0} {f.unit}</span>
                    </span>
                  ...
                  <span className={styles.fuelLastFill}>Last fill: {f.lastFill || 'Not set'}</span>
                  {canEditInventory && (
